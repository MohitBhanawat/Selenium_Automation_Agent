...
import org.smartqa.automationagent.edge.EdgeDriver;
...
import org.smartqa.automationagent.grid.web.NoHandler;
...
import org.smartqa.automationagent.opera.OperaDriver;
...
/**
 * Runs HTML AutomationAgent test suites.
 */
public class HTMLLauncher {

  //    java -jar automationagent-server-standalone-<version-number>.jar -htmlSuite "*firefox"
  ...
  /**
   * Launches a single HTML AutomationAgent test suite.
   *
  ...
  public String runHTMLSuite(
    ...
    try {
      ...
      AutomationAgent automationAgent = new WebDriverBackedAutomationAgent(driver, startURL);
      automationAgent.setTimeout(String.valueOf(timeoutInMs));
      if (userExtensions != null) {
        automationAgent.setExtensionJs(userExtensions);
      }
      ...
      Results results = new CoreTestSuite(suiteUrl.toString()).run(driver, automationAgent, new URL(startURL));
