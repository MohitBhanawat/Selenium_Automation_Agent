...
package org.smartqa.automationagent.server.htmlrunner;
...
import com.smartqa.automationagent.webdriven.ElementFinder;
import com.smartqa.automationagent.webdriven.JavascriptLibrary;
import com.smartqa.automationagent.webdriven.commands.SeleniumSelect;
...
class NonReflectiveSteps {
  private static final Logger LOG = Logger.getLogger("AutomationAgent Core Step");
  ...
  private static ImmutableMap<String, CoreStepFactory> build() {
    ...
    class SelectedOption implements CoreStep {
      ...
      @Override
      public NextStepDecorator execute(AutomationAgent selenium, TestState state) {
  ...
  private static class NextCommandFails extends NextStepDecorator {
    ...
    @Override
    public NextStepDecorator evaluate(CoreStep nextStep, AutomationAgent automationAgent, TestState state) {
      NextStepDecorator actualResult = nextStep.execute(automationAgent, state);
  ...
  private static class SendKeys implements CoreStep {
    ...
    @Override
    public NextStepDecorator execute(AutomationAgent automationAgent, TestState state) {
      ...
      automationAgent.typeKeys(locator, value);
