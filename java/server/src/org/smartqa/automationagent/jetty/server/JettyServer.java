...
import org.smartqa.automationagent.grid.server.AddWebDriverSpecHeaders;
import org.smartqa.automationagent.grid.server.BaseServerOptions;
import org.smartqa.automationagent.grid.server.Server;
import org.smartqa.automationagent.grid.server.WrapExceptions;
...
public class JettyServer implements Server<JettyServer> {
  ...
  public JettyServer(BaseServerOptions options, HttpHandler handler) {
    ...
    // Allow CORS: Whether the AutomationAgent server should allow web browser connections from any host
    if (options.getAllowCORS()) {
      ...
      LOG.warning("You have enabled CORS requests from any host. "
                  ...
                  + "try to start AutomationAgent sessions on your machine");
    }
