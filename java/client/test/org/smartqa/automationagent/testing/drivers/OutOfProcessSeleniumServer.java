...
import org.smartqa.automationagent.net.NetworkUtils;
...
public class OutOfProcessSeleniumServer {
  ...
  public OutOfProcessSeleniumServer start(String mode, String... extraFlags) {
    log.info("Got a request to start a new automationagent server");
    if (command != null) {
    ...
    command.setWorkingDirectory(
        InProjectLocate.locate("Rakefile").getParent().toAbsolutePath().toString());
    log.info("Starting automationagent server: " + command.toString());
  ...
  public void stop() {
    ...
    log.info("Stopping automationagent server");
    command.destroy();
    log.info("AutomationAgent server stopped");
    command = null;
  }

  private String buildServerAndClasspath() {
    new BazelBuild().build("//java/server/src/org/smartqa/automationagent/grid:selenium_server_deploy.jar");
    return InProjectLocate.locate("bazel-bin")
        .resolve("java/server/src/org/smartqa/automationagent/grid/selenium_server_deploy.jar")
