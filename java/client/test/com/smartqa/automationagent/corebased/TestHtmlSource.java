...
public class TestHtmlSource extends InternalSelenseTestBase {
  ...
  public void testHtmlSource() {
    automationAgent.open("test_html_source.html");
    verifyTrue(automationAgent.getHtmlSource().matches("^[\\s\\S]*Text is here[\\s\\S]*$"));
    verifyFalse(automationAgent.getHtmlSource().matches("^[\\s\\S]*can not be found[\\s\\S]*$"));
  }
}
