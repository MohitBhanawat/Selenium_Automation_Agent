...
public class TestAttachFile extends InternalSelenseTestBase {
  ...
  public void testAttachFile() throws Exception {
    automationAgent.open("/common/upload.html");
    automationAgent.attachFile("upload", testFile.toURI().toURL().toString());
    automationAgent.click("go");
    Thread.sleep(2000);
    automationAgent.waitForPageToLoad("30000");
    automationAgent.selectFrame("upload_target");
    assertEquals(automationAgent.getText("//body"), LOREM_IPSUM_TEXT);
  }
  ...
  public void testAttachNonExistingFile() throws Exception {
    automationAgent.open("/common/upload.html");
    try {
      automationAgent.attachFile("upload", testFile.toURI().toURL().toString() + "-missing");
