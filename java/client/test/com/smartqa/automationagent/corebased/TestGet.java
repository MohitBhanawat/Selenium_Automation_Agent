...
public class TestGet extends InternalSelenseTestBase {
  ...
  public void testGet() {
    // test API getters
    automationAgent.open("test_get.html");
    ...
    verifyTrue(Pattern.compile("(width|WIDTH): 644px; (height|HEIGHT): 41px(;?)")
        .matcher(automationAgent.getAttribute("//img[@alt='banner']@style")).find());
    ...
    verifyTrue(join(automationAgent.getSelectOptions("selectWithFunkyValues"), ',').matches(
        "^foo[\\s\\S]*$"));
    // verifyTrue(join(automationagent.getSelectOptions("selectWithFunkyValues"),
    // ',').matches("^javascript\\{ \\[ 'foo', '\\\\,\\\\\\\\\\\\\\\\bar\\\\\\\\\\\\\\\\\\\\,', '[\\s\\S]*baz[\\s\\S]*' \\]\\.join\\(','\\) \\}$"));
    // verifyEquals(join(automationagent.getSelectOptions("selectWithFunkyValues"), ','),
    // automationagent.getEval(" 'regexp:' + [ 'foo', '\\\\\\,\\\\\\\\\\\\\\\\bar\\\\\\\\\\\\\\\\\\\\\\,', '\\\\u00a0{2}baz\\\\u00a0{2}' ].join(',') "));
  }
}
