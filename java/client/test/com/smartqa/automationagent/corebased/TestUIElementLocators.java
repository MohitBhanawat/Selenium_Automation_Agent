...
public class TestUIElementLocators extends InternalSelenseTestBase {
  ...
  public void testUIElementLocators() {
    automationAgent.addScript(getUiMap(), "uimap");
    automationAgent.open("test_locators.html");
    verifyEquals(automationAgent.getText("ui=pageset1::linksWithId()"), "this is the first element");
    verifyEquals(automationAgent.getText("ui=pageset1::linksWithId(index=1)"), "this is the first element");
    verifyTrue(automationAgent.getText("ui=pageset1::linksWithId(index=2)").matches(
        "^this is the[\\s\\S]*second[\\s\\S]*element$"));
    verifyEquals(automationAgent.getText("ui=pageset1::linksWithId(index=3)"), "this is the third element");
    verifyEquals(automationAgent.getText("ui=pageset1::fourthLink()"), "this is the fourth element");
    verifyEquals(automationAgent.getText("ui=pageset1::fifthLink()"), "this is the fifth element");
    verifyEquals(automationAgent.getText("ui=pageset1::linksWithId()->//span"), "element");
    verifyEquals(automationAgent.getText("ui=pageset2::cell(text=theHeaderText)"), "theHeaderText");
    verifyEquals(automationAgent.getText("ui=pageset2::cell(text=theCellText)"), "theCellText");
    verifyEquals(
        automationAgent
            .getEval("map.getUISpecifierString(automationagent.browserbot.findElement('id=firstChild'), automationagent.browserbot.getDocument())"),
        "ui=pageset3::anyDiv()->/child::span");
    automationAgent.removeScript("uimap");
  }
