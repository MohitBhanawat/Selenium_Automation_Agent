...
public class TestCommandError extends InternalSelenseTestBase {
  ...
  public void testCommandError() {
    ...
    try {
      automationAgent.click("notALink");
      fail("expected failure");
    } catch (Throwable e) {
    }
    try {
      automationAgent.select("noSuchSelect", "somelabel");
      fail("expected failure");
    } catch (Throwable e) {
    }
    try {
      automationAgent.select("theSelect", "label=noSuchLabel");
      fail("expected failure");
    } catch (Throwable e) {
    }
    try {
      automationAgent.select("theText", "label=noSuchLabel");
      fail("expected failure");
    } catch (Throwable e) {
    }
  }
