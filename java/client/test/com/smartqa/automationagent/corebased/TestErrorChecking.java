...
public class TestErrorChecking extends InternalSelenseTestBase {
  ...
  public void testErrorChecking() {
    ...
    try {
      assertEquals(automationAgent.getText("link"), "Click here for next page");
    ...
    try {
      assertEquals(automationAgent.getText("link"), "foo");
      fail("expected failure");
    } catch (Throwable e) {
    }
    try {
    ...
    try {
      assertEquals(automationAgent.getText("notAlink"), "foo");
      fail("expected failure");
    } catch (Throwable e) {
    }
  }
