...
/**
 * The default implementation of the AutomationAgent interface; <i>end users will primarily interact with
 * this object.</i>
 *
 * @deprecated The RC interface will be removed in AutomationAgent 3.0. Please migrate to using WebDriver.
 */
@Deprecated
public class DefaultAutomationAgent implements AutomationAgent {
   ...
   * custom Firefox profile. This profile will be automatically configured to use the AutomationAgent
   ...
   * to use the AutomationAgent Server as a proxy and to have all annoying prompts ("save your password?"
   ...
   * the AutomationAgent Server as a proxy, and disable all browser-specific prompting.
   * </ul>
   *
   * @param serverHost the host name on which the AutomationAgent Server resides
   * @param serverPort the port on which the AutomationAgent Server is listening
   ...
   *        pointing at the AutomationAgent resources on this URL, e.g. "http://www.google.com" would send
   *        the browser to "http://www.google.com/selenium-server/SeleneseRunner.html"
   */
  public DefaultAutomationAgent(String serverHost, int serverPort, String browserStartCommand,
                                String browserURL) {
   ...
   */
  public DefaultAutomationAgent(CommandProcessor processor) {
  ...
  public void start() {
    try {
      ...
      if (message != null && message.startsWith("Connection refused")) {
        throw new RuntimeException("Could not contact AutomationAgent Server; have you started it on '" +
      ...
      throw new RuntimeException("Could not start AutomationAgent session: " + e.getMessage(), e);
    }
  }
  ...
  public void start(String optionsString) {
    ...
    // Found)
      ...
      if (message != null && message.indexOf("Connection refused: connect") != -1) {
        throw new RuntimeException("Could not contact AutomationAgent Server; have you started it?\n" +
            e.getMessage());
      }
      throw new RuntimeException("Could not start AutomationAgent session: " + e.getMessage(), e);
    }
