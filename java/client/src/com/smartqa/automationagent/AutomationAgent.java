...
/**
 * Defines an object that runs AutomationAgent commands.
 *
 * <h3><a name="locators"></a>Element Locators</h3>
 * <p>
 * Element Locators tell AutomationAgent which HTML element a command refers to. The format of a locator
 * is:
 * </p>
 ...
 * information. You can also check the TestCssLocators test in the automationagent test suite for an
 * example of usage, which is included in the downloaded automationagent core package.
 * <ul class="first last simple">
 ...
 * "http://svn.openqa.org/fisheye/browse/~raw,r=trunk/automationagent/trunk/src/main/resources/core/scripts/ui-doc.html"
 * >AutomationAgent UI-Element Reference</a> for more details.
 * <ul class="first last simple">
 ...
 * </ul>
 * <p>
 * Without an explicit locator prefix, AutomationAgent uses the following default strategies:
 * </p>
 * <ul class="simple">
 ...
 * </ul>
 * <p>
 * If no pattern prefix is specified, AutomationAgent assumes that it's a "glob" pattern.
 * </p>
 * <p>
 ...
 * </p>
 * @deprecated The RC interface will be removed in AutomationAgent 3.0. Please migrate to using WebDriver.
 */
@Deprecated
public interface AutomationAgent {
  ...
  /** Launches the browser with a new AutomationAgent session */
  void start();

  /** Starts a new AutomationAgent testing session with a String, representing a configuration
   * @param optionsString option string
   */
  void start(String optionsString);

  /** Starts a new AutomationAgent testing session with a configuration options object
   ...
   * automationagent operation). By default, there is no such delay, i.e., the delay is 0 milliseconds.
   *
  ...
  /**
   * Get execution speed (i.e., get the millisecond length of the delay following each automationagent
   ...
   * use the AutomationAgent Server to start a new browser session on that domain.
   *
   ...
   *
   * <p>
   * This command can also be a useful workaround for bug SEL-339. In some cases, AutomationAgent will be
   * unable to intercept a call to window.open (if the call occurs during or before the "onLoad"
   * event, for example). In those cases, you can force AutomationAgent to notice the open window's name by
   * using the AutomationAgent openWindow command, using an empty (blank) url, like this: openWindow("",
   * "myFunnyWindow").
   * </p>
   *
   ...
   * window.open(url, windowName, windowFeatures, replaceFlag) (which AutomationAgent intercepts).</li>
   ...
   * </p>
   * <p>
   * 3.) Otherwise, automationagent looks in a hash it maintains that maps string names to window "names".
   * </p>
   * <p>
   ...
   * the AutomationAgent log messages which identify the names of windows created via window.open (and
   * therefore intercepted by AutomationAgent). You will see messages like the following for each window as
   * it is opened:
   * </p>
   * <p>
   ...
   * </p>
   * <p>
   * In some cases, AutomationAgent will be unable to intercept a call to window.open (if the call occurs
   * during or before the "onLoad" event, for example). (This is bug SEL-339.) In those cases, you
   * can force AutomationAgent to notice the open window's name by using the AutomationAgent openWindow command,
   ...
   * and sometimes the automationagent server needs to identify the "current" frame. In this case, when the
   ...
   * <p>
   * This is useful in proxy injection mode, where this code runs in every browser frame and window,
   * and sometimes the automationagent server needs to identify the "current" window. In this case, when
   ...
   *        this value is not specified, the default AutomationAgent timeout will be used. See the
   *        setTimeout() command.
   */
  ...
  /**
   * <p>
   * By default, AutomationAgent's overridden window.confirm() function will return true, as if the user
   * had manually clicked OK; after running this command, the next call to confirm() will return
   * false, as if the user had clicked Cancel. AutomationAgent will then resume using the default behavior
   ...
   * getConfirmation, or else the next automationagent operation will fail.
   * </p>
   */
  ...
  /**
   * <p>
   * Undo the effect of calling chooseCancelOnNextConfirmation. Note that AutomationAgent's overridden
   ...
   * your mind prior to the next confirmation. After any confirmation, AutomationAgent will resume using
  ...
  /**
   * Instructs AutomationAgent to return the specified answer string in response to the next JavaScript
   ...
   * do not consume it with getAlert, the next AutomationAgent action will fail.
   * </p>
   * <p>
   * Under AutomationAgent, JavaScript alerts will NOT pop up a visible alert dialog.
   * </p>
   * <p>
   * AutomationAgent does NOT support JavaScript alerts that are generated in a page's onload() event
   * handler. In this case a visible dialog WILL be generated and AutomationAgent will hang until someone
   * manually clicks OK.
   * </p>
   *
   ...
   * AutomationAgent action will fail.
   * </p>
   * <p>
   * NOTE: under AutomationAgent, JavaScript confirmations will NOT pop up a visible dialog.
   * </p>
   * <p>
   * NOTE: AutomationAgent does NOT support JavaScript confirmations that are generated in a page's
   * onload() event handler. In this case a visible dialog WILL be generated and AutomationAgent will hang
   ...
   * If a prompt is generated but you do not get/verify it, the next AutomationAgent action will fail.
   * </p>
   * <p>
   * NOTE: under AutomationAgent, JavaScript prompts will NOT pop up a visible dialog.
   * </p>
   * <p>
   * NOTE: AutomationAgent does NOT support JavaScript prompts that are generated in a page's onload()
   * event handler. In this case a visible dialog WILL be generated and AutomationAgent will hang until
   ...
   *
   * <p>
   * Note that, by default, the snippet will run in the context of the "automationagent" object itself, so
   * <code>this</code> will refer to the AutomationAgent object. Use <code>window</code> to refer to the
  ...
  /**
   * Specifies whether AutomationAgent should use the native in-browser implementation of XPath (if any
  ...
  /**
   * Specifies whether AutomationAgent will ignore xpath attributes that have no value, i.e. are the empty
   ...
   * <code>automationagent.browserbot.getCurrentWindow()</code>, and then run your JavaScript in there
   * </p>
   *
   * @param script the JavaScript snippet to run
  ...
  /**
   * Specifies the amount of time that AutomationAgent will wait for actions to complete.
   *
   * <p>
   ...
   * </p>
   * <p>
   * AutomationAgent constantly keeps track of new pages loading, and sets a "newPageLoaded" flag when it
   * first notices a page load. Running any other AutomationAgent command after turns the flag to false.
   * Hence, if you want to wait for a page to load, you must wait immediately after a AutomationAgent
   ...
   *
   * <p>
   * AutomationAgent constantly keeps track of new pages and frames loading, and sets a "newPageLoaded"
   ...
   * scripts executed using AutomationAgent's "getEval" command. Beware that JS exceptions thrown in these
   * script tags aren't managed by AutomationAgent, so you should probably wrap your script in try/catch
  ...
  /**
   * Defines a new function for AutomationAgent to locate elements on the page. For example, if you define
  ...
  /**
   * Loads script content into a new script tag in the AutomationAgent document. This differs from the
   * runScript command in that runScript adds the script tag to the document of the AUT, not the
   * AutomationAgent document. The following entities in the script content are replaced by the characters
   * they represent:
   *
   * &lt; &gt; &amp;
   *
  ...
  /**
   * Removes a script tag from the AutomationAgent document identified by the given id. Does nothing if the
   ...
   *        input field (fieldLocator), AutomationAgent RC may need to transfer the file to the local
   *        machine before attaching the file in a web page form. This is common in automationagent grid
  ...
  /**
   * Tells the AutomationAgent server to add the specificed key and value as a custom outgoing request
   * header. This only works if the browser is configured to use the built in AutomationAgent proxy.
   *
  ...
  /**
   * Kills the running AutomationAgent Server and all browser sessions. After you run this command, you
